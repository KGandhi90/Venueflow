...
export default function Home() {
  ...
  const { seat, dashStats, waitTimes, isInitialized } = useVenue()
  ...
  return (
    <div className="page-enter" style={{ padding: '20px 20px 120px', display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'flex', gap: 8, overflowX: 'auto' }} className="no-scrollbar">
        {!isInitialized ? (
          <>
            <Skeleton width="100px" height="32px" rounded="rounded-full" />
            <Skeleton width="100px" height="32px" rounded="rounded-full" />
            <Skeleton width="100px" height="32px" rounded="rounded-full" />
          </>
        ) : (
        ...
        {!isInitialized ? (
          ...
          quickActions.map(({ id, icon: Icon, title, desc, to }) => (
            <div
              key={id}
              id={id}
              className="pressable hover:border-[rgba(255,255,255,0.15)] transition-colors duration-200"
              onClick={() => navigate(to)}
              style={{
                background: '#111118',
                border: '1px solid rgba(255,255,255,0.07)',
